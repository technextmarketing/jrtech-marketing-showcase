...
(function(){
  ...
  document.querySelectorAll('section,footer').forEach(function(el){
    ...
    if(!/#080C18|#0F1528|#060A15/.test(bg))return;
...
function onScroll(){
  ...
  if(nav)nav.style.background= st>40 ? 'rgba(8,12,24,.9)' : 'rgba(8,12,24,.55)';
  var cur='';
...
// ── Pillar cards scroll to detail sections ──
(function(){
  var ids=['svc-social','svc-content','svc-seo','svc-geo','svc-ads','svc-analytics','svc-reviews'];
